...
def get_method(parameters: dict) -> dict:
    """
    Handles GET requests to fetch document records based on various query parameters.
...
def build_query(parameters: dict) -> tuple:
    """
    Builds the SQL query and parameters for fetching document records.
...
def fetch_documents(
    ...
    """
    Fetches document records with pagination and sorting.
...
@timer
def specific_aircraft_documents(cursor: MySQLCursorAbstract, document_id: int) -> list:
    """
    Returns a list of aircraft linked with a specific document ID.

    Args:
        cursor (MySQLCursorAbstract): The database cursor for executing queries.
        document_id (int): The document ID to query.

    Returns:
        list: The list of aircraft IDs.
    """
    query = "SELECT aircraft_id FROM aircraft_documents WHERE document_id = %s"
